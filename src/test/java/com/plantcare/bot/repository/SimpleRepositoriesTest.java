...
class SimpleRepositoriesTest extends IntegrationTestBase {
    ...
    void plantArchiveExcludesFromActiveListings() {
        ...
        Plant archived = plantRepository.save(Plant.builder()
                .user(user)
                .location(location)
                .name("Архивное")
                .build());
        ...
        assertThat(plantRepository.countByUserIdAndArchivedAtIsNull(user.getId()))
                .isEqualTo(1);
    ...
    void notificationDedupeCheckWorks() {
        ...
        assertThat(notificationRepository.existsByPlantIdAndTaskTypeAndSentAtAfter(
                plant.getId(),
                TaskType.WATERING,
                LocalDateTime.now().truncatedTo(ChronoUnit.MICROS).minusHours(6)
        )).isTrue();

        assertThat(notificationRepository.existsByPlantIdAndTaskTypeAndSentAtAfter(
                plant.getId(),
                TaskType.WATERING,
                LocalDateTime.now().truncatedTo(ChronoUnit.MICROS).minusMinutes(30)
        )).isFalse();
