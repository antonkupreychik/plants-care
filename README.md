...
Деструктивные и изменяющие действия админки пишутся в таблицу `admin_audit_log`:
кто, что, над каким объектом, с каким контекстом (`details` в JSONB) и с какого IP.
Покрыты действия над юзером (сброс состояния, блокировка, пауза/снятие паузы,
сообщение, feature-флаги), CRUD видов и фактов о видах, запуск и останов рассылки,
массовый сброс застрявших.

Смотреть — `/admin/audit`: фильтры по админу, действию, типу объекта и диапазону дат
(даты в UTC), раскрывающийся JSON `details` и выгрузка текущей выборки в CSV
(`/admin/audit/export.csv`, до 10 000 строк). На карточке юзера есть секция
«История админских действий» с последними 10 записями по этому юзеру.

Таблица **append-only**: триггер БД отклоняет `UPDATE`, `DELETE` и `TRUNCATE`,
включая попытки владельца таблицы. Снять защиту можно только явным
`ALTER TABLE admin_audit_log DISABLE TRIGGER USER` — в проде так делать не нужно.

### Photo storage (`/admin/storage`)

Управление фото в S3-совместимом бакете: объём и прикидка расходов, график роста
за полгода, топ юзеров по объёму, последние загрузки с превью. Грид всех фото
конкретного юзера — на его странице `/admin/users/{id}`.

Удаление двухфазное:

1. **Soft-delete** — админ жмёт «удалить» (точечно или «удалить все фото» для
   GDPR-запроса). Запись помечается `deleted_at`, фото сразу недоступно юзеру,
   объект остаётся в бакете и удаление **обратимо**.
2. **Физическая чистка** — суточная задача `StorageMaintenanceScheduler` (03:20 UTC)
   удаляет из бакета объекты, у которых soft-delete старше retention, и ставит
   `purged_at`. После этого возврат невозможен. Кнопка «Вычистить сейчас» на
   странице запускает тот же прогон вручную.

Строка в `photos` не удаляется никогда — на неё ссылается
`plant_progress_photos.photo_id`, и её удаление уронило бы CHECK-инвариант
таймлайна. `purged_at` работает как тумбстоун.

Объём считается по реестру `photos` (`purged_at IS NULL`), а не листингом бакета:
снапшот пишется раз в сутки в `storage_metrics` (03:05 UTC) и питает график роста.

| Env | Дефолт | Что делает |
|---|---|---|
| `PHOTO_RETENTION_DAYS` | `30` | Сколько дней soft-deleted фото живёт до физического удаления |
| `PHOTO_PRICE_PER_GB_MONTH` | `0.015` | Тариф для прикидки расходов на дашборде (USD за ГБ·мес) |
| `PHOTO_PURGE_BATCH_SIZE` | `500` | Максимум объектов за один прогон чистки |
